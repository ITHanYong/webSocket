...
@implementation HYMsgAttributeModel
...
- (void)getAttributedStringFromSelf {
    ...
    switch (self.msgModel.subType) {
        case HYSubMsgType_Subscription: { // 关注
            self.bgColor = NormalBgColor;
            [self Subscription];
        }
            break;
        case HYSubMsgType_Share: { // 分享
            self.bgColor = NormalBgColor;
            [self Share];
        }
            break;
        case HYSubMsgType_At:
        case HYSubMsgType_Comment: { // 弹幕消息
            // 下载标签图片
            ...
            break;
        case HYSubMsgType_MemberEnter: { // 用户进入直播间
            // 下载标签图片
            [self downloadTagImage];
            self.bgColor = NormalBgColor;
            [self MemberEnter];
            ...
            break;
        case HYSubMsgType_Gift_Text: {   // 礼物弹幕(文本)消息
            // 下载标签图片
            [self downloadTagImage];
            // 下载礼物图片
            ...
            break;
        case HYSubMsgType_Announcement: { // 系统公告信息
            self.bgColor = NormalBgColor;
            [self Unknow];
        }
            break;
//        case HYSubMsgType_TimeMsg: { // 客户端提示消息
...
- (void)Comment {
    ...
    // 等级
    NSMutableAttributedString *textView = [self getAttachText:[[HYLevelManager sharedInstance] imageForLevel:self.msgModel.user.level] tap:YES];
