...
@interface HYMsgListTableView()<UITableViewDelegate, UITableViewDataSource, MsgCellGesDelegate> {
    /** 正在滚动(滚动时禁止执行插入动画) */
    BOOL _inAnimation;
    CGFloat _AllHeight;
...
/** 消息数组(数据源) */
@property (nonatomic, strong) NSMutableArray<HYMsgModel *> *msgArray;
/** 用于存储消息还未刷新到tableView的时候接收到的消息 */
@property (nonatomic, strong) NSMutableArray<HYMsgModel *> *tempMsgArray;
/** 底部更多未读按钮 */
@property (nonatomic, strong) HYLayoutButton *moreButton;
...
@implementation HYMsgListTableView
...
- (void)setReloadType:(HYReloadLiveMsgRoomType)reloadType {
    _reloadType = reloadType;
    if (_reloadType == HYReloadLiveMsgRoom_Direct) {
        [self stopTimer];
    }
}
...
- (void)appendAndScrollToBottom {
    ...
    for (HYMsgModel *item in self.tempMsgArray) {
...
// 新消息按钮
- (void)moreClick:(HYLayoutButton *)button {
    [self appendAndScrollToBottom];
    self.inPending = NO;
}
...
//    HYGroupMsgModel *msgModel = [[HYGroupMsgModel alloc] init];
//    msgModel.resource.content = text;
//    msgModel.subType = HYSubMsgType_TimeMsg;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    HYMsgModel *msgModel = self.msgArray[indexPath.row];
    
    HYLiveMsgBassCell *cell = [HYLiveMsgBassCell tableView:tableView cellForMsg:msgModel indexPath:indexPath delegate:self];
...
- (NSMutableArray<HYMsgModel *> *)msgArray {
    if(!_msgArray){
...
- (NSMutableArray<HYMsgModel *> *)tempMsgArray {
    if(!_tempMsgArray){
...
- (HYLayoutButton *)moreButton {
    if (!_moreButton) {
        _moreButton = [HYLayoutButton buttonWithType:UIButtonTypeCustom];
