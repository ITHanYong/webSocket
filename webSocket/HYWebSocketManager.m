...
#define PingIdentifier  @"" //心跳标识：与服务器端约定好

@interface HYWebSocketManager ()<SRWebSocketDelegate>
...
+ (instancetype)shared{
    static HYWebSocketManager *_instance = nil;
...
//建立长连接
- (void)connectServer:(NSString *)liveId{
    self.isActivelyClose = NO;
    self.liveId = liveId;
...
- (void)sendPing:(id)data{
    [self.webSocket sendPing:data error:NULL];
}
...
- (void)reConnectServer{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(self.reConnectTime *NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [weakSelf connectServer:self.liveId];
...
//                }
        ...
        {
            [self connectServer:self.liveId]; //连接服务器
        }
    }
